...
import { FaserObjects } from './objects/faser-objects';
 ...
 */
export class FaserLoader implements EventDataLoader {
  ...
  protected loadObjectTypes(eventData: any) {
    ...
    if (eventData.Tracks) {
      ...
      this.addObjectType(eventData.Tracks, FaserObjects.getTrack, 'Tracks', cuts);
    ...
    if (eventData.Jets) {
      ...
      this.addObjectType(eventData.Jets, FaserObjects.getJet, 'Jets', cuts, addJetsSizeOption);
    }

    if (eventData.Hits) {
      // Cannot currently cut on just a postion array.
      this.addObjectType(eventData.Hits, FaserObjects.getHits, 'Hits');
    }

    if (eventData.LargeScintHits) {
      // Cannot currently cut on just a postion array.
    ...
    if (eventData.CaloClusters) {
      ...
      this.addObjectType(eventData.CaloClusters, FaserObjects.getCluster, 'CaloClusters', cuts);
    }
    ...
    //   this.addObjectType(eventData.Photons, FaserObjects.getPhotons, 'Muons');
    // }

    // if (eventData.Electrons) {
    //   this.addObjectType(eventData.Photons, FaserObjects.getElectrons, 'Muons');
    // }
    ...
    if (eventData.Vertices) {
      ...
      this.addObjectType(eventData.Vertices, FaserObjects.getVertex, 'Vertices', cuts);
    }
  }
    ...
    if ('LinkedClusters' in muonParams) {
      for (const clusterID of muonParams.LinkedClusters) {
        ...
        if (clusterColl && clusterIndex && this.eventData.CaloClusters && this.eventData.CaloClusters[clusterColl]) {
          ...
          if (clusterParams) {
            const cluster = FaserObjects.getCluster(clusterParams);
    ...
    if ('LinkedTracks' in muonParams) {
      for (const trackID of muonParams.LinkedTracks) {
        ...
        if (trackColl && trackIndex && this.eventData.Tracks && this.eventData.Tracks[trackColl]) {
          ...
          if (trackParams) {
            const track = FaserObjects.getTrack(trackParams);
            muonScene.add(track);
          }
        }
      }
